...
class ScreenProtector {
  ...
  addScreenshotDetection() {
    try {
      ...
      // 监听右键菜单（可能的截屏软件操作）
      this.addEventListenerSafe(document, 'contextmenu', (event) => {
        console.log('🔍 检测到右键菜单操作')
        this.handleScreenshotDetected()
      })

      // 监听选择事件（可能的截屏软件选择区域）
      this.addEventListenerSafe(document, 'selectstart', (event) => {
        console.log('🔍 检测到文本选择操作')
        this.handleScreenshotDetected()
      })

      // 监听拖拽事件（可能的截屏软件拖拽操作）
      this.addEventListenerSafe(document, 'dragstart', (event) => {
        console.log('🔍 检测到拖拽操作')
        this.handleScreenshotDetected()
      })
      ...
      this.addEventListenerSafe(window, 'beforeunload', () => {
        console.log('🔍 页面即将卸载，可能存在截屏行为')
        this.handleScreenshotDetected()
      })

      console.log('📸 增强截屏检测已启用')
    } catch (error) {
