...
// 暂时禁用echarts导入，使用简单图表工具替代
let echarts = null

// 异步加载echarts - 暂时禁用
async function loadEcharts() {
  console.warn('ECharts已禁用，请使用简单图表工具')
  return null
}
 ...
 */
export async function initChart(canvasId, option, context) {
  try {
      ...
      try {
        ...
        setTimeout(() => {
          try {
            ...
            const chart = echartsLib.init(ctx, null, {
              width: 300,
              height: 200,
              devicePixelRatio: 1
            })
            ...
            try {
              const ctx = uni.createCanvasContext(canvasId, context)
              const chart = echartsLib.init(ctx)
              chart.setOption(option)
              chart.render()
