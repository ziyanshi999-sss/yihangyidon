...
class UnifiedScreenProtector {
    ...
    async trySimpleDetection() {
        ...
        try {
            // 创建内置的简化保护器
            const simpleProtector = this.createSimpleProtector()
            ...
            if (result) {
                ...
                simpleProtector.on('onRecordingDetected', (data) => {
        ...
        return {
            ...
            async enable() {
                try {
                    ...
                    this.isEnabled = true
                    console.log('✅ 简化防录屏保护器启用成功')
                    return true
                } catch (error) {
                    console.error('❌ 简化防录屏保护器启用失败:', error)
                    return false
                }
            },
            ...
            handlePossibleScreenshot() {
                console.log('🚨 检测到可能的截屏行为')
                this.triggerCallback('onScreenshotDetected', {
            ...
            handlePossibleRecording() {
                console.log('🚨 检测到可能的录屏行为')
                this.triggerCallback('onRecordingDetected', {
                    type: 'possible_recording',
                    timestamp: Date.now(),
                    method: 'simplified_detection'
                })
            },
