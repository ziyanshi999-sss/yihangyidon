...
<script>
import { getForexRates, getCachedData } from '@/api/crawler.js'
import { generateForexChart } from '@/api/chartGenerator.js'
import { initChart, createForexChart } from '@/utils/echarts.js'
import { createChartUpdater, addChartAnimation } from '@/utils/chartUpdater.js'
  ...
  methods: {
    ...
    async loadForexData() {
      try {
        ...
        // 获取外汇数据
        const forexData = await getCachedData('forex_rates', getForexRates)
        ...
        // 生成图表
        const chartConfig = await generateForexChart(forexData)
