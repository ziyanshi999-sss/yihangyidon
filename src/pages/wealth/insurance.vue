...
<script>
import { getInsuranceProducts, getCachedData } from '@/api/crawler.js'
import { generateInsuranceChart } from '@/api/chartGenerator.js'
import { initChart, createInsuranceChart } from '@/utils/echarts.js'
  ...
  methods: {
    ...
    async loadInsuranceData() {
      try {
        ...
        // 获取保险产品数据
        const insuranceData = await getCachedData('insurance_products', getInsuranceProducts)
        console.log('保险产品数据:', insuranceData)
        
        // 保存数据用于图表渲染
        this.insuranceData = insuranceData
        
        // 更新市场数据
        if (insuranceData.marketInsights) {
          this.marketData = {
            totalPremium: (insuranceData.marketInsights.totalPremium / 100000000).toFixed(0),
            growthRate: insuranceData.marketInsights.growthRate,
            productCount: insuranceData.categories.reduce((sum, cat) => sum + cat.products.length, 0)
          }
        }
        
        // 生成图表
        const chartConfig = await generateInsuranceChart(insuranceData)
