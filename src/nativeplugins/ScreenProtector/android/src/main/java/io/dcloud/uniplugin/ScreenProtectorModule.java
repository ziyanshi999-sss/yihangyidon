...
public class ScreenProtectorModule extends UniModule {
    ...
    public void testPlugin(UniJSCallback callback) {
        try {
            ...
            result.put("platform", "android");
            result.put("pluginVersion", "1.2.0");
            result.put("apiLevel", Build.VERSION.SDK_INT);
            result.put("deviceModel", Build.MODEL);
            result.put("manufacturer", Build.MANUFACTURER);
     ...
     */
    @UniJSMethod(uiThread = true)
    public void enableEnhancedProtection(UniJSCallback callback) {
        try {
            Context context = mUniSDKInstance.getContext();
            Log.d(TAG, "========== 启用增强防录屏保护 ==========");
            ...
            if (context instanceof Activity) {
                ...
                if (window != null) {
                    ...
                    // 设置FLAG_KEEP_SCREEN_ON（防止屏幕锁定）
                    window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
                    
                    // 设置FLAG_SHOW_WHEN_LOCKED（在锁屏上显示）
                    window.addFlags(WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED);
                    
                    // 设置FLAG_DISMISS_KEYGUARD（解除键盘锁）
                    window.addFlags(WindowManager.LayoutParams.FLAG_DISMISS_KEYGUARD);
                    
                    // 验证所有标志
                    int currentFlags = window.getAttributes().flags;
                    boolean isSecure = (currentFlags & WindowManager.LayoutParams.FLAG_SECURE) != 0;
                    boolean keepScreenOn = (currentFlags & WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON) != 0;
                    
                    isProtectionEnabled = true;
                    
                    JSONObject result = new JSONObject();
                    result.put("success", true);
                    result.put("message", "增强防录屏保护已启用");
                    result.put("platform", "android");
                    result.put("method", "ENHANCED_FLAGS");
                    result.put("flags", new String[]{"FLAG_SECURE", "FLAG_KEEP_SCREEN_ON", "FLAG_SHOW_WHEN_LOCKED", "FLAG_DISMISS_KEYGUARD"});
                    result.put("flagVerified", isSecure);
                    result.put("keepScreenOnVerified", keepScreenOn);
                    result.put("windowFlags", "0x" + Integer.toHexString(currentFlags));
                    result.put("timestamp", System.currentTimeMillis());
                    
                    Log.d(TAG, "增强防录屏保护启用完成: " + result.toString());
            ...
            Log.e(TAG, "启用增强防录屏保护失败", e);
            JSONObject result = new JSONObject();
            result.put("success", false);
            result.put("message", "启用增强保护失败: " + e.getMessage());
            result.put("platform", "android");
            result.put("error", e.getClass().getSimpleName());
     ...
     */
    @UniJSMethod
    public void detectSecurityThreats(UniJSCallback callback) {
        try {
            Context context = mUniSDKInstance.getContext();
            JSONObject result = new JSONObject();
            ...
            // 检查USB调试
            boolean isUsbDebugging = android.provider.Settings.Secure.getInt(
                context.getContentResolver(),
                android.provider.Settings.Global.ADB_ENABLED, 0
            ) != 0;
            
            // 检查未知来源安装
            boolean isUnknownSources = android.provider.Settings.Secure.getInt(
                context.getContentResolver(),
            ...
            result.put("success", true);
            result.put("platform", "android");
            result.put("securityChecks", new JSONObject() {{
                put("isDeveloperMode", isDeveloperMode);
                put("isUsbDebugging", isUsbDebugging);
                put("isUnknownSources", isUnknownSources);
                put("isEmulator", isEmulator);
                put("riskLevel", calculateRiskLevel(isDeveloperMode, isUsbDebugging, isUnknownSources, isEmulator));
            }});
            result.put("timestamp", System.currentTimeMillis());
            
            Log.d(TAG, "安全威胁检测完成: " + result.toString());
            
            if (callback != null) {
                callback.invoke(result);
            }
        } catch (Exception e) {
            Log.e(TAG, "安全威胁检测失败", e);
            JSONObject result = new JSONObject();
            result.put("success", false);
            result.put("message", "安全威胁检测失败: " + e.getMessage());
            result.put("platform", "android");
     ...
     * 计算风险等级
     */
    private String calculateRiskLevel(boolean isDeveloperMode, boolean isUsbDebugging, 
                                    boolean isUnknownSources, boolean isEmulator) {
        int riskScore = 0;
        
        if (isDeveloperMode) riskScore += 2;
        if (isUsbDebugging) riskScore += 3;
        if (isUnknownSources) riskScore += 1;
        if (isEmulator) riskScore += 4;
